...
namespace YoutubeToMusic.BLL
{
    public class YT_DLP
    {
        public static async Task<(bool, string)> DownloadVideo(string outputTemplate, string url)
        {
            var psi = new ProcessStartInfo
            {
                FileName = "yt-dlp", // or full path if needed
                Arguments = $"-f bestaudio -x --audio-format opus -o \"{outputTemplate}\" \"{url}\"",
            ...
            {
                return (false, $"yt-dlp failed (exit {process.ExitCode})\n{stderr}");
            }

            return (true, stdout.ToString());
        }
